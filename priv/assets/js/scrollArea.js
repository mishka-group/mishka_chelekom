...
const ScrollArea = {
    mounted() {
      ...
      this.viewport.scrollTop  = parseInt(sessionStorage.getItem("scrollTop")  || 0, 10);
      this.viewport.scrollLeft = parseInt(sessionStorage.getItem("scrollLeft") || 0, 10);
      ...
      this.viewport.addEventListener("touchstart", this.boundTouchstart, { passive: false });
      this.viewport.addEventListener("touchmove", this.boundTouchmove, { passive: false });
    ...
    handleKeyboardScroll(e) {
      ...
      this.scrollInterval = setInterval(() => {
        ...
        switch (e.key) {
          case "ArrowUp":    scrollBy({ top: -step });      break;
          case "ArrowDown":  scrollBy({ top: step });       break;
          case "ArrowLeft":  scrollBy({ left: -step });     break;
          case "ArrowRight": scrollBy({ left: step });      break;
          case "PageUp":     scrollBy({ top: -clientHeight });  break;
          case "PageDown":   scrollBy({ top: clientHeight });   break;
          default: break;
        }
        this.updateThumbPosition();
    ...
    initThumbDrag(axis) {
      ...
      thumb.addEventListener("mousedown", (e) => {
        ...
        const startScroll = isVertical ? this.viewport.scrollTop : this.viewport.scrollLeft;
  
        // We'll store these so we can remove them in `destroyed()`
        const onMouseMove = (evt) => {
          const delta = (isVertical ? evt.clientY : evt.clientX) - startCoord;
          const ratio = delta / (isVertical ? this.viewport.clientHeight : this.viewport.clientWidth);
          const contentSize = isVertical ? this.content.scrollHeight : this.content.scrollWidth;
