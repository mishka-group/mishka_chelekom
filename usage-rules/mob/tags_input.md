...
| `wrap_chars` | number | `28` — characters per token row |
...
budget (each tag costs its length plus a fixed allowance for its padding and ✕). It is tuned so
three short tags share a line the way the web's do. Raise it for a wide control, lower it for a
narrow one; a tag longer than the whole budget gets a row to itself rather than being dropped.
