...
An unstyled one-time-code input — a row of single-character boxes with auto-advance, paste
...
pasting a code distributes its characters across the boxes. A `chelekom:complete` DOM event
...
```
Attrs: `id` (req), `name` (hidden input name), `length` (default 6), `class`, `rest`.

## Styling
Ships no colors. Lay the boxes out and size them via the `chelekom-otp_field__input` class
(the root is `display:inline-flex; gap` by default).
