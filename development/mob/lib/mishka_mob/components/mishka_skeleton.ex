defmodule MishkaMob.Components.MishkaSkeleton do
  ...
  **Ordinary nodes cannot.** `Box`, `Text`, `Row` and `Column` expose no opacity,
  ...
  only self-animating things the bridge draws are Material's two progress
  indicators, which `MishkaMob.Components.MishkaProgress` wraps.
  ...
  **`Mob.UI.gpu_view/1` can.** It is a fragment-shader surface — GLES 3.0 on
  ...
  `MishkaMob.Components.MishkaLoadingOverlay` already animate for free.

  (There is no animation plugin to reach for either: as of mob 0.7 the published
  `mob_*` packages are all capabilities — camera, location, biometric, scanner,
  bluetooth, nfc, vision, video, notify, photos, touch, screencast, background,
  audio_capture — plus `mob_themes`, `mob_ash` and `mob_push`.)
