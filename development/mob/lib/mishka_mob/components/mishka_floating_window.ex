defmodule MishkaMob.Components.MishkaFloatingWindow do
  ...
  defp handle(props) do
    ...
    # The Text takes `font_weight` and the Spacer below it takes `weight`: two
    # different props. A bare `weight` on the Text would be read by the Row as a
    # layout weight and do nothing to the font.
    ~MOB"""
    <Box fill_width={true} background={:surface_raised} padding={:space_sm}>
      <Row fill_width={true} align={:center}>
        <Text text={label} text_size={:sm} text_color={:on_surface} font_weight={:semibold} />
        <Spacer weight={1} />
        {nudges(props)}
        {close(props)}
      </Row>
