defmodule MishkaMob.Components.MishkaSplitter do
  ...
  defp grip_ops(width, height, axis, props, extent, grip) do
    ...
    if axis == :vertical do
      [
        Mob.Canvas.rect(0, at - grip / 2, width, grip, color: @grip_band, radius: 2),
        Mob.Canvas.rect(width / 2 - 14, at - 1.5, 28, 3, color: ink, radius: 2)
      ]
    else
      [
        Mob.Canvas.rect(at - grip / 2, 0, grip, height, color: @grip_band, radius: 2),
        Mob.Canvas.rect(at - 1.5, height / 2 - 14, 3, 28, color: ink, radius: 2)
      ]
    end
  end
