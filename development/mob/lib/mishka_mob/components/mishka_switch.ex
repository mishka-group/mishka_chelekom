defmodule MishkaMob.Components.MishkaSwitch do
  ...
  ## Controlled, like every Mob widget

  `Toggle` is controlled: it renders whatever `checked` says and reports
  ...
  def switch(props \\ %{}) do
    ...
    node = ~MOB(<Toggle value={checked} />)

    node
    |> put_prop(:label, Map.get(props, :label))
