defmodule MishkaMob.Components.MishkaDrawer do
  ...
  **canvas-local** coordinates, so re-rendering the panel at a new height on
  ...
  `trigger/2` builds the opener — the port of the web's `<:trigger>` slot.
  ...
  `sheet-description`, `sheet-close`, `sheet-handle`, `sheet-swipe-area`, and
  `sheet-snap-<n>` — the 1-based index of the active snap point, on the handle's
  row. That last one exists because the active snap is otherwise only a panel
  height, and a device test that measures a height in px cannot say which snap
  point produced it.
  ...
  **Verified on Android (Samsung A55, One UI / Android 15)** — all four sides
  ...
  read it). Returns a Mob node map — the overlay tree when open, the edge
  swipe-area when closed and `swipe_area` is on, and an empty column otherwise.
  """
  ...
  @doc """
  The thing that opens the drawer — the port of the web's `<:trigger>` slot.
  ...
  The web renders this inside the drawer's own markup; here it stays where you
  put it, because the composite has no way to reach into the caller's layout —
  and because a drawer whose trigger lived inside it could not be opened from
  two places.
  """
  ...
  defp overlay(props, children, ctx) do
    positioner = positioner(side(props), panel(props, children, ctx))
  ...
  # Handle, header and the caller's children. The handle sits at the panel's
  # free edge — the top for a bottom sheet or a side drawer, the bottom for a
  # top sheet — which is the edge a thumb reaches for.
  defp body(props, children, ctx) do
    content = header(props, ctx) ++ children
