defmodule MishkaMob.Components.MishkaJsonInput do
  ...
  when `lines > 1`). Note that the return key then inserts a newline rather than
  submitting, which is the correct behaviour for a textarea and means `on_submit`
  is only meaningful on a single-line field.
  ...
  def json_input(props \\ %{}) do
    ...
    field =
      ...
      <TextField
        ...
        background={:surface}
        padding={:space_sm}
        border_color={if(invalid?, do: :danger, else: :border)}
        border_width={1}
      />
      """
  ...
  defp message(props, result, invalid?) do
    with true <- truthy?(Map.get(props, :show_error, true)),
      ...
      <Column fill_width={true}>
        <Spacer size={6} />
        <Text text={text} text_size={:sm} text_color={:danger} />
      </Column>
      """
    else
      _ -> nil
  ...
  defp from_result(_), do: "Invalid JSON"
