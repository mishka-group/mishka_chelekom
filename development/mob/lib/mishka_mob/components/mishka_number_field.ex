defmodule MishkaMob.Components.MishkaNumberField do
  ...
  | `decimals` | integer | derived from `step` | How the value is rendered. |

  Not ported: `name` (form plumbing), `small_step` / `large_step` (modifier keys),
  `snap_on_step`, `allow_out_of_range` (the port always clamps), scrub-to-change
  (a pointer gesture), and `id` / `*_class`.
  """
  ...
  def number_field(props \\ %{}) do
    ...
    text = to_text(Map.get(props, :value), decimals)

    ~MOB"""
    <Row fill_width={true}>
      {stepper("−", :down, props, disabled?)}
      <Spacer size={8} />
      <Box weight={1}>
        {input(props, text, disabled?)}
      </Box>
      <Spacer size={8} />
      {stepper("+", :up, props, disabled?)}
    </Row>
    """
  end
  ...
  defp stepper(glyph, direction, props, disabled?) do
    ...
    case handler(props, :on_step, disabled?) do
      nil -> node
