defmodule DevelopmentWeb.Components.Headless.NavLink do
  ...
  **Documentation:** https://mishka.tools/chelekom/docs/headless/nav_link
  ...
  attr :control_class, :any, default: nil, doc: ~s|Extra classes for `data-part="control"`|
  ...
  attr :trailing_class, :any, default: nil, doc: ~s|Extra classes for `data-part="trailing"`|
  ...
  def nav_link(assigns) do
    ...
    >
      <summary
        data-part="control"
        phx-click={@on_toggle}
        class={["chelekom-nav-link__control", @control_class]}
      >
        <span
          :if={@icon != []}
          data-part="icon"
          class={["chelekom-nav-link__icon", @icon_class]}
        >{render_slot(@icon)}</span>
        <span data-part="label" class={["chelekom-nav-link__label", @label_class]}>{@label ||
          render_slot(@inner_block)}</span>
        <span
          :if={@trailing != []}
          data-part="trailing"
          class={["chelekom-nav-link__trailing", @trailing_class]}
        >{render_slot(@trailing)}</span>
      </summary>
