defmodule DevelopmentWeb.Components.Headless.Popover do
  ...
  **Documentation:** https://mishka.tools/chelekom/docs/headless/popover
  """
  ...
  attr :backdrop_class, :any, default: nil, doc: ~s|Extra classes for `data-part="backdrop"`|
  ...
  slot :arrow, doc: "Optional arrow pointing at the trigger (rendered inside the popup)"
  ...
  def popover(assigns) do
    ...
    >
      ...
      <div
        :if={@modal != false}
        data-part="backdrop"
        class={["chelekom-popover__backdrop", @backdrop_class]}
        aria-hidden="true"
      >
      </div>
      ...
      >
        ...
        <div :if={@close != []} data-part="footer" class={["chelekom-popover__footer", @footer_class]}>
          {render_slot(@close)}
        </div>
      </div>
    </div>
