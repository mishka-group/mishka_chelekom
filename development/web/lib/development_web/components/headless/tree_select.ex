defmodule DevelopmentWeb.Components.Headless.TreeSelect do
  ...
  `panel` with JS commands (no hook) — `aria-expanded` follows, and clicking outside or pressing
  Escape closes it — plus a default slot where you drop a `<.tree>` (generate the `tree` component
  alongside this one). Wire the tree's `on_select` to your LiveView to update the shown label and a
  hidden form field — the tree owns all of its own keyboard and selection behaviour. Pass `open` to
  render the panel expanded.
  ...
  **Documentation:** https://mishka.tools/chelekom/docs/headless/tree_select
  ...
  def tree_select(assigns) do
    ...
    <div
      ...
      phx-key="escape"
      class={["chelekom-tree-select", @class]}
      {@rest}
    >
      <button
        type="button"
      ...
      >
        <span
          data-part="value"
          data-placeholder={@label in [nil, ""] && "true"}
          class={["chelekom-tree-select__value", @value_class]}
        >
        ...
        <span
          data-part="caret"
          aria-hidden="true"
          class={["chelekom-tree-select__caret", @caret_class]}
        >▾</span>
      </button>
      <div
        id={"#{@id}-panel"}
