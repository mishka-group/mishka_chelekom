defmodule DevelopmentWeb.Components.Headless.AngleSlider do
  ...
  value with a form — the hidden input fires `input` on each commit, so a wrapping
  `<.form phx-change>` sees every change — and/or an `on_change` event name to receive it on the
  server (pushed on release and on each key change).
  ...
  **Documentation:** https://mishka.tools/chelekom/docs/headless/angle_slider
  ...
  attr :thumb_layer_class, :any,
    default: nil,
    doc: ~s|Extra classes for `data-part="thumb-layer"`|
  ...
  def angle_slider(assigns) do
    ...
    <div
      ...
      {@rest}
    >
      <span data-part="thumb-layer" class={["chelekom-angle-slider__thumb-layer", @thumb_layer_class]}>
        <span data-part="thumb" class={["chelekom-angle-slider__thumb", @thumb_class]}></span>
      </span>
      <span :if={@show_value} data-part="value" class={["chelekom-angle-slider__value", @value_class]}>{@value}°</span>
