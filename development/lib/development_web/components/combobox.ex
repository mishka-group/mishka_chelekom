defmodule DevelopmentWeb.Components.Combobox do
  @moduledoc """
  The `DevelopmentWeb.Components.Combobox` is a Phoenix LiveView component module for creating customizable combobox elements.
  ...
  **Documentation:** https://mishka.tools/chelekom/docs/forms/combobox
  ...
  attr :creatable, :boolean, default: false, doc: "Enables creating new options from search input"
  attr :create_label, :string, default: "Create", doc: "Label prefix for the create option button"

  attr :on_create, :string,
    default: nil,
    doc: "Event name pushed to the server when a new option is created"
  ...
  slot :option, required: false do
    ...
    attr :class, :any, doc: "Value of the select option"
  ...
  def combobox(%{multiple: true} = assigns) do
    assigns =
      ...
      |> then(fn a -> if a.creatable, do: assign(a, :searchable, true), else: a end)
    ...
    <div class={[
      ...
      <div
        ...
        data-creatable={@creatable || nil}
        data-create-label={@create_label}
          ...
          >
            ...
            >
              <div class="px-1.5">
                ...
                <div
                  :if={@creatable}
                  class="combobox-create-option combobox-option cursor-pointer rounded flex items-center gap-2"
                  data-part="create-option"
                  role="option"
                  hidden
                >
                  <icon name="hero-plus" class="size-4 shrink-0 combobox-icon" />
                  <span class="combobox-create-label"></span>
                </div>
              </div>
            </.scroll_area>
          </div>
  ...
  attr :class, :any, default: nil, doc: "Custom class"
  ...
  defp translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(DevelopmentWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(DevelopmentWeb.Gettext, "errors", msg, opts)
    end
  end
end
