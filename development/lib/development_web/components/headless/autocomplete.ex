defmodule DevelopmentWeb.Components.Headless.Autocomplete do
  ...
  Structurally identical to the headless `combobox`; behavior via the shared
  `HeadlessCombobox` JS engine: typing in the `input` filters the options (non-matching
  items get `data-hidden`), ArrowDown opens and moves a roving highlight
  (`data-highlighted` + `aria-activedescendant`), Enter/click selects (filling the input and
  a hidden `value` input), and Escape closes. The engine wires `role="combobox"`,
  `aria-controls`, `aria-expanded` and `aria-autocomplete` on the input at runtime; the
  listbox is `role="listbox"` and options are `role="option"`. Style via `chelekom-autocomplete*`
  ...
  def autocomplete(assigns) do
    ~H"""
    <div id={@id} phx-hook="HeadlessCombobox" class={["chelekom-autocomplete", @class]} {@rest}>
      <input
        type="text"
        data-part="input"
