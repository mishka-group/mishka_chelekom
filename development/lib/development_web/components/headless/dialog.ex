defmodule DevelopmentWeb.Components.Headless.Dialog do
  @moduledoc """
  Headless **dialog** (modal) — unstyled markup + WAI-ARIA wiring + focus management.

  Behavior is delegated to the shared `FocusTrap` JS engine: the `<:trigger>` opens the
  dialog, focus is trapped inside the popup, and Escape / backdrop / `<:close>` close it,
  restoring focus to the opener. You can also drive `open` from the server.

  Anatomy (named slots / parts): `trigger`, `backdrop` (auto), `popup`, `title`,
  `description`, inner content, `close`. Style via the `chelekom-dialog*` classes and the
  ...
  def dialog(assigns) do
    ...
    >
      ...
      <div data-part="backdrop" class="chelekom-dialog__backdrop" aria-hidden="true"></div>
