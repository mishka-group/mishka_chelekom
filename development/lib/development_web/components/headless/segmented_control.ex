defmodule DevelopmentWeb.Components.Headless.SegmentedControl do
  ...
  keys) are all native — **no JS**. Visually hide the radios and show the selected segment with the
  CSS `:has(:checked)` selector. `options` is a list of values (strings) or `{label, value}` tuples.
  ...
  **Documentation:** https://mishka.tools/chelekom/docs/headless/segmented_control
  ...
  attr :options, :list, required: true, doc: "List of values (strings) or {label, value} tuples"
