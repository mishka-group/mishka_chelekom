defmodule DevelopmentWeb.Components.Headless.Toggle do
  ...
  **Optional form submission** (a Phoenix extension over Base UI): give it a `name` and the toggle
  ...
  attr :value, :string, default: nil, doc: "The toggle's value — its group identity and (with name) the value submitted when on"
  attr :name, :string, default: nil, doc: "Optional: submit the toggle as a form field (hidden checkbox)"
