defmodule DevelopmentWeb.Components.Headless.Toolbar do
  ...
  attr :focusable_when_disabled, :boolean, default: true, doc: "Keep disabled items in the roving order"
  ...
  def toolbar(assigns) do
    first =
      Enum.find(assigns.item, fn it ->
        (it[:type] || "button") in ~w(button link input) and not (it[:disabled] || assigns.disabled)
      end)
