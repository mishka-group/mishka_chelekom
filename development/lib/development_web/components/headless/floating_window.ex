defmodule DevelopmentWeb.Components.Headless.FloatingWindow do
  ...
  Drag the `handle` to move the window; the `FloatingWindow` JS hook clamps it inside the offset
  parent, mirrors the position into `data-x` / `data-y`, and (when `on_move` is set) pushes the new
  coordinates to the server on release. Interactive controls inside the handle (like a close button)
  don't start a drag.
  ...
  **Documentation:** https://mishka.tools/chelekom/docs/headless/floating_window
  ...
  def floating_window(assigns) do
    ...
    <div
      ...
      {@rest}
    >
      <div data-part="handle" class={["chelekom-floating-window__handle", @handle_class]}>
        {render_slot(@handle)}
      </div>
