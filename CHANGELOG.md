...
- Add headless `CloseButton` component (Mantine parity) — an icon-only `<button>` with a required `aria-label`, a built-in ✕ or custom icon, and `data-disabled`; wired into the showcase and Base UI gallery
